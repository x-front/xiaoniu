...
<script type="text/javascript">
	...
	function initUpdateNewsWindow(index){
		...
		row = rows[index];
		var id = row['id'];
		$.post("/secure/imageNews/queryImageNewsByNewsId",{'newsId':id,'${_csrf.parameterName}':'${_csrf.token}'},function(result){
	...
	function updateLang(lang){
		...
		if(isSelected(rows)){
			...
			$.post("/secure/imageNews/batchUpdateHeadImageNewsLang?strIds="+ids,{'lang':lang,'${_csrf.parameterName}':'${_csrf.token}'},function(result){
				if(result.resultCode == 0){
