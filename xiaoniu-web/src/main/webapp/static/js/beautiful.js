	...
	$.post("/pageInfo/beauty",function(result){
		if(result.resultCode == 0){
			if(result.family){
				...
				$(".beautiful-f").css('background-image','url('+result.family.extCover+')');
			}
			
			if(result.sports){
				$(".beautiful-t .beautiful-t-inner p:eq(0)").html(result.sports.introdution);
				$(".beautiful-t").css('background-image','url('+result.sports.extCover+')');
			}
			
			if(result.welfare){
				$(".beautiful-g .beautiful-g-inner p:eq(0)").html(result.welfare.introdution);
				$(".beautiful-g").css('background-image','url('+result.welfare.extCover+')');
			}
			
			if(result.education){
				$(".beautiful-j .beautiful-j-inner p:eq(0)").html(result.education.introdution);
				$(".beautiful-j").css('background-image','url('+result.education.extCover+')');
			}
