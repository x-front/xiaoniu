...
function _xn_init(){
	$.post("/news/list",{'type':4,'page':1,'rows':1,'top':1},function(result){
		if(result.resultCode == 0 && result.total > 0){
			...
			$(".about-w-banner a:eq(0)").attr('href','/static/beautiful-j-inside.html?id='+head.id);
		}
	},"json");
	seeMore();
}
...
function seeMore(){
	...
	if(page >= 0){
		$.post("/news/list",{'type':4,'page':page/2+1,'rows':2,'top':0},function(result){
...
function buildNewsDiv(entity,index){
	...
	var html = '<div class="about-w-item about-w-item'+(index%2 + 1)+'"><a href="/static/beautiful-j-inside.html?id='+entity.id +'"><div class="about-wh-img wow fadeInUp"><img src="' + entity.banner +'"/></div>';
	html += '<h3 class="wow fadeInUp">' + entity.title +'</h3>';
	html += '<p class="wow fadeInUp">' + entity.summary + '</p></a>';
	html += '<a href="/static/beautiful-j-inside.html?id='+entity.id+'" class="more wow fadeInUp">See more<span></span></a>';
	html += '</div>';
	return html;
}
