...
function _xn_init(){
	$.post("/news/list",{'type':1,'page':1,'rows':1,'top':1},function(result){
		if(result.resultCode == 0 && result.total > 0){
			...
			$(".about-w-link").attr('href','/static/beautiful-f-inside.html?id='+head.id);
		}
	},"json");
	seeMore();
}
...
function buildNewsDiv(entity,index){
	...
	var html = '<div class="about-w-item about-w-item'+(index%2 + 1)+' wow fadeInUp"><a href="/static/beautiful-f-inside.html?id='+entity.id +'"><div class="about-wh-img"><img src="' + entity.banner +'"/></div>';
	html += '<h3>' + entity.title +'</h3>';
	html += '<p>' + entity.summary + '</p></a>';
	html += '<a href="/static/beautiful-f-inside.html?id='+entity.id+'" class="more">See more<span></span></a>';
	html += '</div>';
	return html;
}
