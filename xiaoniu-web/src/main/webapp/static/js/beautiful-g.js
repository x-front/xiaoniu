...
function _xn_init(){
	$.post("/news/list",{'type':3,'page':1,'rows':1,'top':1},function(result){
		if(result.resultCode == 0 && result.total > 0){
			...
			$(".about-w-banner a:eq(0)").attr('href','/static/beautiful-g-inside.html?id='+head.id);
		}
	},"json");
	seeMore();
}
...
function seeMore(){
	...
	if(page >= 0){
		$.post("/news/list",{'type':3,'page':page/2+1,'rows':2,'top':0},function(result){
