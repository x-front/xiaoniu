;(function (d) {
    ...
    d.fn.bxSlider = function (b) {
        ...
        var a = {}, c = this, e = d(window).width(), g = d(window).height(), l = function () {
            ...
            a.usingCSS = a.settings.useCSS && "fade" != a.settings.mode && function () {
                    var f = document.createElement("div"), b = ["WebkitPerspective", "MozPerspective", "OPerspective", "msPerspective"], m;
                    for (m in b)if (void 0 !== f.style[b[m]])return a.cssPrefix = b[m].replace("Perspective", "").toLowerCase(), a.animProp = "-" + a.cssPrefix + "-transform", !0;
                    return !1
                }();
            ...
            a.viewport.parent().css({maxWidth: A()});
            ...
            if (a.settings.infiniteLoop && "fade" != a.settings.mode && !a.settings.ticker) {
                var f = "vertical" == a.settings.mode ? a.settings.minSlides : a.settings.maxSlides, b = a.children.slice(0, f).clone().addClass("bx-clone"), f = a.children.slice(-f).clone().addClass("bx-clone");
            ...
            if ("vertical" == a.settings.mode || a.settings.adaptiveHeight)if (a.carousel) {
                var m = 1 == a.settings.moveSlides ? a.active.index : a.active.index * h(), b = a.children.eq(m);
                for (i = 1; i <= a.settings.maxSlides - 1; i++)b = m + i >= a.children.length ? b.add(a.children.eq(i - 1)) : b.add(a.children.eq(m + i))
            } else b = a.children.eq(a.active.index); else b = a.children;
            ...
            var f = a.settings.slideWidth, b = a.viewport.width();
            ...
            var f = 0;
            if (0 < a.settings.moveSlides)if (a.settings.infiniteLoop)f = Math.ceil(a.children.length / h()); else for (var b = 0, p = 0; b < a.children.length;)++f, b = p + m(), p += a.settings.moveSlides <= m() ? a.settings.moveSlides : m(); else f = Math.ceil(a.children.length / m());
            return f
        }, h = function () {
            ...
            if (a.children.length > a.settings.maxSlides && a.active.last && !a.settings.infiniteLoop)if ("horizontal" == a.settings.mode) {
                var f = a.children.last(), b = f.position();
                w(-(b.left - (a.viewport.width() - f.outerWidth())), "reset", 0)
            } else"vertical" == a.settings.mode && (b = a.children.eq(a.children.length - a.settings.minSlides).position(), w(-b.top, "reset", 0)); else b = a.children.eq(a.active.index * h()).position(), a.active.index == p() - 1 && (a.active.last = !0), void 0 != b && ("horizontal" == a.settings.mode ? w(-b.left, "reset", 0) : "vertical" == a.settings.mode && w(-b.top, "reset", 0))
        }, w = function (f, b, m, h) {
            ...
            w("horizontal" == a.settings.mode ? -f.left : -f.top, "ticker", speed, {resetValue: "horizontal" == a.settings.mode ? -b.left : -b.top})
        }, N = function (f) {
            ...
            var b = f.originalEvent, m = Math.abs(b.changedTouches[0].pageX - a.touch.start.x), h = Math.abs(b.changedTouches[0].pageY - a.touch.start.y);
        ...
        c.goToSlide = function (f, b) {
            if (!a.working && a.active.index != f) {
                ...
                if ("next" == b)a.settings.onSlideNext(a.children.eq(a.active.index), a.oldIndex, a.active.index); else if ("prev" == b)a.settings.onSlidePrev(a.children.eq(a.active.index), a.oldIndex, a.active.index);
                a.active.last = a.active.index >= p() - 1;
                a.settings.pager && C(a.active.index);
                a.settings.controls && F();
                if ("fade" == a.settings.mode)a.settings.adaptiveHeight && a.viewport.height() != x() && a.viewport.animate({height: x()}, a.settings.adaptiveHeightSpeed), a.children.filter(":visible").fadeOut(a.settings.speed).css({zIndex: 0}), a.children.eq(a.active.index).css("zIndex", a.settings.slideZIndex + 1).fadeIn(a.settings.speed, function () {
                    d(this).css("zIndex", a.settings.slideZIndex);
                    D()
                }); else {
                    a.settings.adaptiveHeight && a.viewport.height() != x() && a.viewport.animate({height: x()}, a.settings.adaptiveHeightSpeed);
                    var m = 0, k = {left: 0, top: 0};
                    if (!a.settings.infiniteLoop && a.carousel && a.active.last)if ("horizontal" == a.settings.mode)var s = a.children.eq(a.children.length - 1), k = s.position(), m = a.viewport.width() - s.outerWidth(); else k = a.children.eq(a.children.length - a.settings.minSlides).position(); else a.carousel && a.active.last && "prev" == b ? (k = 1 == a.settings.moveSlides ? a.settings.maxSlides - h() : (p() - 1) * h() - (a.children.length - a.settings.maxSlides), s = c.children(".bx-clone").eq(k), k = s.position()) : "next" == b && 0 == a.active.index ? (k = c.find("> .bx-clone").eq(a.settings.maxSlides).position(), a.active.last = !1) : 0 <= f && (k = f * h(), k = a.children.eq(k).position());
...
(function (d) {
    ...
    d.extend(d.easing, {
        def: "easeOutQuad", swing: function (t, b, a, c, e) {
    ...
    this.WOW = function () {
        ...
        return c.prototype.defaults = {
            ...
            if (this.stopped = !1, this.boxes = function () {
                    ...
                    for (f = c.length; f > b; b++)a = c[b], p.push(a);
                    return p
                }.call(this), this.boxes.length)if (this.disabled())this.resetStyle(); else for (e = this.boxes, b = 0, c = e.length; c > b; b++)a = e[b], this.applyStyle(a, !0);
...
!function () {
    ...
    t.prototype = {
        addData: function (a) {
            ...
            for (var c = -1, h = this.moduleCount - 1, d = 7, e = 0, g = this.moduleCount - 1; 0 < g; g -= 2)for (6 == g && g--; ;) {
    ...
    t.createBytes = function (a, c) {
        for (var d = 0, h = 0, e = 0, g = Array(c.length), y = Array(c.length), n = 0; n < c.length; n++) {
            var r = c[n].dataCount, l = c[n].totalCount - r, h = Math.max(h, r), e = Math.max(e, l);
            g[n] = Array(r);
    ...
    for (var g = {MODE_NUMBER: 1, MODE_ALPHA_NUM: 2, MODE_8BIT_BYTE: 4, MODE_KANJI: 8}, l = {
        L: 1,
        M: 0,
        Q: 3,
        H: 2
        ...
        getLostPoint: function (a) {
            for (var b = a.getModuleCount(), c = 0, d = 0; b > d; d++)for (var e = 0; b > e; e++) {
                for (var g = 0, k = a.isDark(d, e), n = -1; 1 >= n; n++)if (!(0 > d + n || d + n >= b))for (var r = -1; 1 >= r; r++)0 > e + r || e + r >= b || (0 != n || 0 != r) && k == a.isDark(d + n, e + r) && g++;
                5 < g && (c += 3 + g - 5)
            }
            for (d = 0; b - 1 > d; d++)for (e = 0; b - 1 > e; e++)g = 0, a.isDark(d, e) && g++, a.isDark(d + 1, e) && g++, a.isDark(d, e + 1) && g++, a.isDark(d + 1, e + 1) && g++, 0 != g && 4 != g || (c += 3);
    ...
    a.getRSBlocks = function (b, c) {
        ...
        for (var e = d.length / 3, g = [], w = 0; e > w; w++)for (var k = d[3 * w + 0], n = d[3 * w + 1], r = d[3 * w + 2], l = 0; k > l; l++)g.push(new a(n, r));
        return g
    };
    ...
    var x = [[17, 14, 11, 7], [32, 26, 20, 14], [53, 42, 32, 24], [78, 62, 46, 34], [106, 84, 60, 44], [134, 106, 74, 58], [154, 122, 86, 64], [192, 152, 108, 84], [230, 180, 130, 98], [271, 213, 151, 119], [321, 251, 177, 137], [367, 287, 203, 155], [425, 331, 241, 177], [458, 362, 258, 194], [520, 412, 292, 220], [586, 450, 322, 250], [644, 504, 364, 280], [718, 560, 394, 310], [792, 624, 442, 338], [858, 666, 482, 382], [929, 711, 509, 403], [1003, 779, 565, 439], [1091, 857, 611, 461], [1171, 911, 661, 511], [1273, 997, 715, 535], [1367, 1059, 751, 593], [1465, 1125, 805, 625], [1528, 1190, 868, 658], [1628, 1264, 908, 698], [1732, 1370, 982, 742], [1840, 1452, 1030, 790], [1952, 1538, 1112, 842], [2068, 1628, 1168, 898], [2188, 1722, 1228, 958], [2303, 1809, 1283, 983], [2431, 1911, 1351, 1051], [2563, 1989, 1423, 1093], [2699, 2099, 1499, 1139], [2809, 2213, 1579, 1219], [2953, 2331, 1663, 1273]], u = function () {
        ...
        return a.prototype.draw = function (a) {
            function b(a, c) {
                var d = document.createElementNS("http://www.w3.org/2000/svg", a), e;
            ...
            for (c = 0; e > c; c++)for (d = 0; e > d; d++)if (a.isDark(c, d)) {
                var g = b("use", {x: String(c), y: String(d)});
        ...
        function b(a, c) {
            ...
            if (d._fFail = c, d._fSuccess = a, null === d._bSupportDataURI) {
                var e = document.createElement("img"), f = function () {
        ...
        if (this._android && 2.1 >= this._android) {
            ...
            CanvasRenderingContext2D.prototype.drawImage = function (a, b, e, f, g, m, k, l) {
                if ("nodeName" in a && /img/i.test(a.nodeName))for (var s = arguments.length - 1; 1 <= s; s--)arguments[s] *= c; else"undefined" == typeof l && (arguments[1] *= c, arguments[2] *= c, arguments[3] *= c, arguments[4] *= c);
        ...
        return g.prototype.draw = function (a) {
            var b = this._elImage, c = this._oContext, d = this._htOption, e = a.getModuleCount(), f = d.width / e, h = d.height / e, g = Math.round(f), m = Math.round(h);
            b.style.display = "none";
            this.clear();
            for (b = 0; e > b; b++)for (var p = 0; e > p; p++) {
                var k = a.isDark(b, p), l = p * f, s = b * h;
    ...
    QRCode.prototype.makeCode = function (a) {
        var b = this._htOption.correctLevel, c = 1, d;
